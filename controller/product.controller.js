...
class ProductController {
    ...
    async getAll(req, res) {
        const dbRes = await pool.execute(`select id,
                                                 productName,
                                                 unit,
                                                 price,
                                                 subcategoryId,
                                                 (select subcategory from subcategories where id = subcategoryId) as subcategory,
                                                 (select category
                                                  from categories
                                                  where id =
                                                        (select categoryId
                                                         from subcategories where id = subcategoryId)  ) as category,
                                                 (select id
                                                  from categories
                                                  where id =
                                                        (select categoryId
                                                         from subcategories where id = subcategoryId)  ) as categoryId
                                          from products`)
